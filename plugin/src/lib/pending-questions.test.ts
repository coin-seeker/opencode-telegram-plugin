...
function samplePending(expiresAt: number): PendingQuestionState {
  return {
    ...
    questions: [
      { header: "Pick", question: "Choose?", options: [{ label: "Yes", description: "Confirm" }] },
    ],
    sentAt: Date.now(),
    expiresAt,
...
describe("pending question store", () => {
  ...
  test("finds by request id and awaiting custom", async () => {
    ...
    assert.equal((await store.findByRequestID("req-1", "ses-1"))?.shortHash, "abc");
    assert.equal(
      (await store.findByRequestID("req-1", "ses-1", "http://localhost:4096/"))?.shortHash,
      "abc",
    );
    assert.equal(await store.findByRequestID("req-1", "ses-other"), undefined);
    assert.equal(
      await store.findByRequestID("req-1", "ses-1", "http://localhost:5099/"),
      undefined,
    );
  ...
  test("scopes hash by session and server", () => {
    const bare = createQuestionShortHash("req-1");
    const scoped = createQuestionShortHash("req-1", "ses-1", "http://localhost:4096/");
    const otherSession = createQuestionShortHash("req-1", "ses-2", "http://localhost:4096/");
    assert.notEqual(bare, scoped);
    assert.notEqual(scoped, otherSession);
    assert.equal(scoped.length, 10);
  });
  ...
  test("preserves unanswered slots as null after JSON roundtrip", async () => {
    const store = createPendingQuestionStore({
      tokenHash: "tok",
      baseDir: join(dir, "null-slots"),
    });
    const pending = samplePending(Date.now() + 60_000);
    pending.questions = [
  ...
  test("normalizes legacy undefined JSON slots to null", async () => {
    ...
    await writeFile(
      join(store.dir, "abc.json"),
      JSON.stringify({ ...pending, answersInProgress: [["A"], undefined] }),
      "utf8",
    );
    assert.deepEqual((await store.loadPending("abc"))?.answersInProgress, [["A"], null]);
  });
